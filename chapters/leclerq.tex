...
\section{2. The verb final [e] problem in transcription}%2
...
\ex%2
    \label{ex:leclerq:2}
\gll   {A} {quelle} {heure} {est-ce}~{que} {le} {musée} \textbf{{ouvrer}}?\\
       At            what time    Q              the museum open\\
       (p. 286)
\ex%3
    \label{ex:leclerq:3}
\gll          {A} {quelle} {heure} {est-ce}~{que} {le} {musée} \textbf{{fermer}}?\\
...
\ea%4
    \label{ex:leclerq:4}
\ea \gll {Je} {parler} {avec} {administration}\\
...
\ex
\gll {On} {aller} {au} {centre}~{d’achats}\\
    we go.\textsc{inf} to mall\\
    (Jill, G3)

\ex
\gll {Il} {retourner} {à} {la} {maison}\\
    he return.\textsc{inf} to the house\\
    (Sandra, G4) (p. 363)
\z
\z

He justifies his choice by stating that similar constructions also occur with verbs of the second (\textit{ouvrir}) or third (\textit{boire}) group, as in \REF{ex:leclerq:5} a, b, c and are also found in his data with negation as in \REF{ex:leclerq:6}:
...
\ex
\gll {Il} {prendre} {des} {vêtements}\\
    he take.\textsc{inf} some clothes\\
    (John, G4) (p. 361)

\ex
\gll {…} {quand} {ils} {ouvrir} {les} {cadeaux}\\
...
\ex {Comment} {tu} {vas} {arrive} {à} {mon} {travail} ?\\
    how         you go.\textsc{s} to my work\
    (Jen, G2)

\ex
\gll {…} {qui} {uh} j’ ai {rencontre} {à} {Nouvelle} {Ecosse}\\
...
\ex%8
    \label{ex:leclerq:8}
\gll         {le} {mère} {et} {le} {petit} {garçon} {[sorte]} {les} {villes}\\
         ‘the mother and the little boy go\_out the city’\\

\ex%9
    \label{ex:leclerq:9}
\gll         {les} {enfants} {et} {le} {grand-mère} {[di]} {au} {revoir}\\
...
Within this framework, the learner’s tasks are the following: \REF{ex:leclerq:1} noticing and acquiring the means that the target language provides for the expression of S-finiteness, and \REF{ex:leclerq:2} finding out whether there are grammaticalised means to express (M) finiteness. Development from S (i.e., discursive or lexical means) to M (i.e., morphological means) finiteness denotes a progression towards native norm, as illustrated by these examples from the ESF project: (examples \REF{ex:leclerq:10} and \REF{ex:leclerq:11} come from \citealt{Véronique2004}, example \REF{ex:leclerq:12} comes from \citealt{Granget2015}).
